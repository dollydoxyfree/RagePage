...
function Form1(props) {
  ...
  useEffect(() => {
    if (props.relationships.length > 0 && params.id) {
      const foundRelationships = props.relationships.find((rel) => params.id === rel.id)
  ...
  const handleSubmit = async (e) => {
    ...
    if (params.id) {
      const reviewURL = `${baseURL1}/${params.id}`
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      <textarea value={situation} onChange={(e) => setSituation(e.target.value)} /><br/><br/>
      <label>I WISH I could of </label>
      <input value={dreamReact} onChange={(e) => setDreamReact(e.target.value)} /><br /><br />
      <label>Reality</label>
      <input value={reality} onChange={(e) => setReality(e.target.value)} /><br /><br />
      <label>name or alias</label>
      <input value={anonAuthor} onChange={(e) => setAnonAuthor(e.target.value)}/> <br/><br/>
